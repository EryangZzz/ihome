...
def get_sqlalchemy_uri(DATABASE):
    # 定义sqlalchemy配置数据库字符串的拼接
    # 返回mysql+pymysql://root:123456@127.0.0.1:3306/flask7格式
    user = DATABASE['USER']
...
def create_validate_code(size=(120, 30),
                         ...
                         fg_color=(18, 18, 18),
                         font_size=20,
                         font_type='C:\Windows\Fonts\DroidSerif.ttf',
                         length=4,
                         draw_lines=True,
                         n_line=(1, 2),
                         draw_points=True,
...
def is_login(func):
    ...
    def check_status(*args, **kwargs):
        ...
        path = request.path
        if not user_id and path not in not_check_path:
